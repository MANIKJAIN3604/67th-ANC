...
const TOAST_LIMIT = 1;
const TOAST_REMOVE_DELAY = 10000; // Reduced delay for better UX
...
type Action =
  ...
  | { type: typeof ActionType.UPDATE_TOAST; toast: Partial<ToasterToast> }
...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case ActionType.DISMISS_TOAST: {
      const { toastId } = action;
      (toastId ? [toastId] : state.toasts.map((t) => t.id)).forEach(addToRemoveQueue);
